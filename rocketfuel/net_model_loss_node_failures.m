...
while (selectednode < numofnodes) %% BEGIN_WHILE
    ...
    if (failurecount == 0)
        ...
        % select the next node and brake it down
        selectednode = selectednode + 1;

        if (edgenodes(selectednode)) %% if an edgenode, don't fail it
            continue; % continues on the WHILE loop
        end;

        % brake down all the links attached to the selected node
        TempAdj(selectednode,:) = 0;
        TempAdj(:,selectednode) = 0;
        
        %% check if the new topology is connected
%         [connected, NumofComponents, GlobalRoot, GlobalParent, GlobalCycle]=Dfs(TempAdj);
%         connected
%         if (~connected) %% if not connected, try failing another node
%             continue; % continues on the WHILE loop
%         end;
        TempCost(selectednode,:) = Inf;
        TempCost(:,selectednode) = Inf;
    ...
    for i=1:numofnodes
        for j=1:numofnodes
            if (i ~= j  &&  i ~= selectednode  &&  j ~= selectednode)
                flowid = (i-1)*numofnodes + j;
                introuter = Prev(i,j);
                last = j;
    ...
    for u = urange
        ...
        for E2E_p_avg = E2E_p_avg_range
            ...
            [t1,t2] = size(find(Q<=0));
            if (failurecount == 1) % if there was no failure
                linkexcesscapacitypercentage(counter,ulevel,failurecount) = 100.0 * sum(ExcessCapacities ./ Capacities) / (numoflinks-t1);
            else
                linkexcesscapacitypercentage(counter,ulevel,failurecount) = 100.0 * sum(ExcessCapacities ./ Capacities) / (numoflinks-t1-sum(Adj(selectednode,:)));
            end;
...
for j=2:failurecount
    ...
    fname = ['../../results/isp-failures/',ispname,'/avg-additional-rec-',num2str(j-1),'.txt'];
    dlmwrite(fname, avgextrarec, '-append', 'delimiter', ' ');
end;

% calculate and record the maximum rec
for i=2:failurecount
    failurerecs{i-1} = linkexcesscapacitypercentage(:,:,i);
end;

maxfailurerec = linkexcesscapacitypercentage(:,:,1);
...
for i=1:failurecount
    fname = ['../../results/isp-failures/',ispname,'/avg-g-',num2str(i-1),'.txt'];
    dlmwrite(fname, avggs(:,:,i), '-append', 'delimiter', ' ');
end;

% record the link utilizations after each failure
for i=1:failurecount
    fname = ['../../results/isp-failures/',ispname,'/link-utilizations-',num2str(i-1),'.txt'];
...
for i=1:failurecount
    maxlinkutilizations(i) = max(individuallinkutilizations{i});
    minlinkutilizations(i) = min(individuallinkutilizations{i});
    avglinkutilizations(i) = mean(individuallinkutilizations{i});
end;
