...
isp_pops = {}
with open('population_coverage.json', 'r') as f:
...
#     affinity_score_isp_a = 1 - (polygons[isp_a_name].area / total_coverage_area_of_two_isps) 
#     if affinity_score_isp_a < 0:
#         affinity_score_isp_a = 0.0
#     affinity_score_isp_b = 1 - (polygons[isp_b_name].area / total_coverage_area_of_two_isps) 
...
def isp_overlap_score(isp_a_as, isp_b_as, isp_a_pop_locations_list, isp_b_pop_locations_list, common_pop_locations):
    '''
    @note: takes all the PoP locations and generates a Convex-Hull for each ISPs.
    Based on the Convex-hull points, generates the polygons. These polygons are the coverage areas.
    Using, shapely, we calculate the intersection (common areas)
    ...
    itersection_pop = isp_pops['polygon'][isp_a_as]['overlap'][isp_b_as]
    total_coverage_pop_of_two_isps = isp_pops['polygon'][isp_a_as]['population'] + isp_pops['polygon'][isp_b_as]['population'] - itersection_pop
    affinity_score_isp_a = 1 - (isp_pops['polygon'][isp_a_as]['population'] / total_coverage_pop_of_two_isps)
    if affinity_score_isp_a < 0:
        affinity_score_isp_a = 0.0
    affinity_score_isp_b = 1 - (isp_pops['polygon'][isp_b_as]['population'] / total_coverage_pop_of_two_isps)
    if affinity_score_isp_b < 0:
...
#         print("0 APC for {} and {}. No APC folder created!".format(isp_a.as_number, isp_b.as_number))
    ...
    affinity_score_combined = isp_overlap_score(str(isp_a.as_number), str(isp_b.as_number), isp_a_pop_locations_list, isp_b_pop_locations_list, common_pop_locations)
...
#             ensure_isp_json_files(isp_pair_list)
