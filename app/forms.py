...
class CustomPeeringQuerryForm(Form):
    ...
    asn1 = FileField('document', validators=[FileRequired()])
    ...
    def validate(self):
        ...
        if not FlaskForm.validate(self):
            return False
        asn1_val = self.asn1_string.data
        asn2_val = self.asn2.data

        if not re.search("^AS[0-9]+$", asn1_val):
            self.asn1_string.errors.append("The ASN format should be ASXXXX")
            valid = False
        ...
        if asn1_val == "":
            self.asn1_string.errors.append("Please select ASN1.")
            valid = False
        if asn2_val == "":
            self.asn1_string.errors.append("Please select ASN2.")
            valid = False
        if asn1_val == asn2_val:
            self.asn1_string.errors.append(
                "ASN1 and ANS2 can not be same. Please choose different ASN1."
            )
            self.asn2.errors.append(
                "ASN1 and ANS2 can not be same. Please choose different ASN2."
